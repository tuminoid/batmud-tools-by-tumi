...
&copyreinc(param('id'), param('copy')) if (param('copy'));
...
my $quicklink			= qq|<a href="reincsim/$SCRIPT?id=$id">Reinc $id</a>|;
my $newreinc			= qq|<a href="reincsim/$SCRIPT?id=$alt_id">Create a new reinc</a>|;
my $copyreinc			= qq|<a href="reincsim/$SCRIPT?id=$alt_id&amp;copy=$id">Copy this to new id ($alt_id)</a>|;
my $viewlink			= qq|<a href="reincsim/viewreinc.pl?id=$id">Viewreinc $id</a>|;
...
<h2 id="reincsim-title">Reinc simulator by Tumi, $VERSION</h2>
...
sub calculateRacialTrainMax {
	...
	#print STDERR "calculateRacialTrainMax: starting\n";
	#print STDERR "stat '",$stat||"(undef)","', race '",$race||"(undef)","'\n";

	#return (0,0) unless ($stat && $race && defined $racial_training{$race});
	return (0,0) unless ($stat && $race);

	#print STDERR "calculateRacialTrainMax: asking for '$race' and '$stat'\n";

	# con, str, int, wis, dex
	#my @maxes = split/,/, $racial_training{$race};
	#my $index = 0; # con
	#$index = 1 if ($stat eq "str");
	#$index = 2 if ($stat eq "int");
	#$index = 3 if ($stat eq "wis");
	#$index = 4 if ($stat eq "dex");

	my @raceinfo = &getRaceInfo($race);
	my $racmax = int($raceinfo[$infoindexhash{$stat}]);
	my $starts = int($racmax * 0.72);

	# print STDERR "calculateRacialTrainMax: race '$race', index '$index', rac '$racmax', st '$starts'\n";
...
sub makeTitle {
	...
	&setLayoutItem('title', "Level $t_l $t_r $t_g1$t_g2");
}
